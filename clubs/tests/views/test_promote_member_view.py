...
class PromoteMemberViewTestCase(TestCase, CreateClubs):
    ...
    def setUp(self):
        ...
        self.url = reverse('promote_club_member', kwargs={'user_id': self.target_user.id,'club_id':self.club.id})

    def test_promote_member_url(self):
        self.assertEqual(self.url,f'/promote_club_member/club_id_{self.club.id}/user_id_{self.target_user.id}')

    def test_promote_member_redirects_when_not_logged_in(self):
        url = reverse('promote_club_member', kwargs={'user_id': self.target_user.id,'club_id':self.club.id})
        redirect_url = reverse_with_next('log_in', url)
        response = self.client.get(url)
        self.assertRedirects(response, redirect_url, status_code=302, target_status_code=200)

    def test_owner_can_promote_member_to_officer(self):
    ...
    def test_owner_cannot_promote_member_with_invalid_id(self):
        self.assertTrue(self.club.is_owner(self.owner))
        self.client.login(username=self.owner.username, password='Password123')
        url = reverse('promote_club_member', kwargs={'user_id': self.target_user.id+9999, 'club_id': self.club.id})
        response = self.client.get(url, follow=True)
        self.target_membership.refresh_from_db()
        response_url = reverse('user_list', kwargs={'club_id': self.club.id})
        self.assertRedirects(response, response_url, status_code=302, target_status_code=200)

    def test_owner_cannot_promote_themselves(self):
        self.assertTrue(self.club.is_owner(self.owner))
        self.client.login(username=self.owner.username, password='Password123')
        url = reverse('promote_club_member', kwargs={'user_id': self.owner.id, 'club_id':self.club.id})
        response = self.client.get(url, follow=True)
    ...
    def test_owner_cannot_promote_an_inactive_member_to_officer(self):
        self.assertTrue(self.club.is_owner(self.owner))
        self.assertTrue(self.target_membership.is_member())
    ...
    def test_applicant_cannot_promote_a_member_to_officer(self):
    ...
    def test_officer_cannot_promote_a_member_to_officer(self):
    ...
    def test_member_cannot_promote_themselves(self):
        ...
        self.client.login(username=self.user.username, password='Password123')
        url = reverse('promote_club_member', kwargs={'user_id': self.user.id, 'club_id': self.club.id})
        response = self.client.get(url, follow=True)
        self.membership.refresh_from_db()
        self.target_membership.refresh_from_db()
        response_url = reverse('user_list', kwargs={'club_id': self.club.id})
